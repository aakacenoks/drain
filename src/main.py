...
def search(udid):
    if devices.cycle_mode:
        ...
        if udid in connected_devices:
            ...
            return {'error': f'device ({udid}) is connected, but could not be disconnected'}, 404
        return {'error': f'device ({udid}) is not connected to any of the hubs ({devices.hubs})'}, 404
    return {'error': 'search is only allowed during cycle mode'}, 405
